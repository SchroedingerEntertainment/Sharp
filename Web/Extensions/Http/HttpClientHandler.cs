...
namespace System.Net.Http
{
    public class HttpClientHandler : HttpMessageHandler
    {
        ...
        private static Action<WebHeaderCollection, string, string> addHeaderWithoutValidateDelegate;
        private static Action<WebHeaderCollection, string, string> AddHeaderWithoutValidate
        {
            get
            {
                if (addHeaderWithoutValidateDelegate == null)
                {
                    MethodInfo AddHeaderWithoutValidateMI = typeof(WebHeaderCollection).GetMethod(
                        "AddWithoutValidate",
                        BindingFlags.NonPublic | BindingFlags.Instance,
                        null,
                        new Type[] { typeof(string), typeof(string) },
                        null);
                    ParameterExpression webHeaderCollectionParameter = Expression.Parameter(typeof(WebHeaderCollection));
                    ParameterExpression headerParameter = Expression.Parameter(typeof(string));
                    ParameterExpression valueParameter = Expression.Parameter(typeof(string));
                    LambdaExpression AddHeaderWithoutValidateLambda = Expression.Lambda(
                        Expression.Call(webHeaderCollectionParameter, AddHeaderWithoutValidateMI, headerParameter, valueParameter),
                        webHeaderCollectionParameter,
                        headerParameter,
                        valueParameter);
                    addHeaderWithoutValidateDelegate = (Action<WebHeaderCollection, string, string>)AddHeaderWithoutValidateLambda.Compile();
                }

                return addHeaderWithoutValidateDelegate;
            }
        }
#endif
            ...
            set
            {
                ...
                if (!UseCookies)
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture,
                        ErrorCodes.NetHttpInvalidEnableFirst, "UseCookies", "true"));
                }
                CheckDisposedOrStarted();
        ...
        public ClientCertificateOption ClientCertificateOptions
        {
            ...
            set
            {
                if (!Enum.IsDefined(typeof(ClientCertificateOption), value))
                {
        ...
        public IWebProxy Proxy
        {
            ...
            set
            {
                ...
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture,
                        ErrorCodes.NetHttpInvalidEnableFirst, "UseProxy", "true"));
                }
                CheckDisposedOrStarted();
                ...
                if (headers.Contains(HttpKnownHeaderNames.ContentLength))
                {
                    foreach (var header in request.Content.Headers)
                    {
                        if (string.CompareOrdinal(HttpKnownHeaderNames.ContentLength, header.Key) != 0)
                        {
                            // Use AddInternal() to skip validation.
#if net40
                            AddHeaderWithoutValidate(webRequest.Headers, header.Key, string.Join(", ", header.Value));
#else
            ...
            CancellationToken cancellationToken)
        {
            if (request == null)
            {
                throw new ArgumentNullException("request", ErrorCodes.NetHttpHandlerNorequest);
            }
            CheckDisposed();
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw new HttpRequestException(ErrorCodes.NetHttpHandlerNocontentlength);
                        }

                        // HttpContent couldn't calculate the content length. Chunked is not specified. Buffer the 
                        // content to get the content length.
                        requestContent.LoadIntoBufferAsync(maxRequestContentBufferSize).ContinueWith(task =>
                        {
                            if (task.IsFaulted)
                            {
        ...
        private void HandleAsyncException(RequestState state, Exception e)
        {
            ...
            {
                state.tcs.TrySetException(new HttpRequestException(ErrorCodes.NetHttpClientExecutionError, e));
            }
            else
            {
        ...
        internal void CheckDisposedOrStarted()
        {
            ...
            if (operationStarted)
            {
                throw new InvalidOperationException(ErrorCodes.NetHttpClientOperationStarted);
            }
        }
        ...
        private class WebExceptionWrapperStream : DelegatingStream
        {
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                {
                    throw new IOException(ErrorCodes.NetHttpReadError, wex);
                }
            }
