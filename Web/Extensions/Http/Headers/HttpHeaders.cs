...
namespace System.Net.Http.Headers
{
    ...
    public abstract class HttpHeaders : IEnumerable<KeyValuePair<string, IEnumerable<string>>>
    {
        ...
        public IEnumerable<string> GetValues(string name)
        {
            ...
            {
                throw new InvalidOperationException(ErrorCodes.NetHttpHeadersNotFound);
            }
        ...
        private static void ParseMultipleRawHeaderValues(string name, HeaderStoreItemInfo info, List<string> rawValues)
        {
            ...
            else
            {
                foreach (string rawValue in rawValues)
                {
                    ...
                    {
                        if (Logging.On) Logging.PrintWarning(Logging.Http, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpLogHeadersInvalidValue, name, rawValue));
                    }
                }
            }
        }
        ...
        private void ParseAndAddValue(string name, HeaderStoreItemInfo info, string value)
        {
            ...
            if (!info.CanAddValue)
            {
                throw new FormatException(string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpHeadersSingleValueHeader, name));
            }
        ...
        private void CheckHeaderName(string name)
        {
            ...
            {
                throw new FormatException(ErrorCodes.NetHttpHeadersInvalidHeaderName);
            }

            if ((invalidHeaders != null) && (invalidHeaders.Contains(name)))
            {
                throw new InvalidOperationException(ErrorCodes.NetHttpHeadersNotAllowedHeaderName);
            }
        }
        ...
        private static void CheckInvalidNewLine(string value)
        {
            ...
            {
                throw new FormatException(ErrorCodes.NetHttpHeadersNoNewlines);
            }
        }
        ...
        private static bool ContainsInvalidNewLine(string value, string name)
        {
            if (HttpRuleParser.ContainsInvalidNewLine(value))
            {
                if (Logging.On) Logging.PrintError(Logging.Http, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpLogHeadersNoNewlines, name, value));
                return true;
            }
            return false;
        }
