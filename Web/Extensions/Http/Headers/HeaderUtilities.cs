    ...
    internal static class HeaderUtilities
    {
        ...
        internal static double? GetQuality(ICollection<NameValueHeaderValue> parameters)
        {
            ...
            if (qualityParameter != null)
            {
                ...
                if (Logging.On) Logging.PrintError(Logging.Http, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpLogHeadersInvalidQuality, qualityParameter.Value));
            }
            return null;
        }
        ...
        internal static void CheckValidToken(string value, string parameterName)
        {
            ...
            {
                throw new ArgumentException(ErrorCodes.NetHttpArgumentEmptyString, parameterName);
            }

            if (HttpRuleParser.GetTokenLength(value, 0) != value.Length)
            {
                throw new FormatException(string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpHeadersInvalidValue, value));
            }
        }
        ...
        internal static bool IsValidEmailAddress(string value)
        {
            ...
            {
                if (Logging.On) Logging.PrintError(Logging.Http, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpLogHeadersWrongEmailFormat, value, e.Message));
            }
            return false;
        }
