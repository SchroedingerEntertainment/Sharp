        ...
        private static void ValidateBoundary(string boundary)
        {
            ...
            if (boundary.Length > 70)
            {
                throw new ArgumentOutOfRangeException("boundary", boundary,
                    string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpContentFieldTooLong, 70));
            }
            // Cannot end with space
            if (boundary.EndsWith(" ", StringComparison.Ordinal))
            {
                throw new ArgumentException(string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpHeadersInvalidValue, boundary), "boundary");
            }
            Contract.EndContractBlock();
