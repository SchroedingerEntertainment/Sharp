    ...
    public abstract class HttpContent : IDisposable
    {
        ...
        public Task<string> ReadAsStringAsync()
        {
            ...
            LoadIntoBufferAsync().ContinueWith(task =>
            {
                ...
                if ((Headers.ContentType != null) && (Headers.ContentType.CharSet != null))
                {
                    ...
                    {
                        tcs.TrySetException(new InvalidOperationException(ErrorCodes.NetHttpContentInvalidCharset, e));
                        return;
                    }
                }
        ...
        private MemoryStream CreateMemoryStream(int maxBufferSize, out Exception error)
        {
            ...
            if (contentLength != null)
            {
                ...
                {
                    error = new HttpRequestException(string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpContentBuffersizeExceeded, maxBufferSize));
                    return null;
                }
...
#region Helpers
        ...
        private void CheckTaskNotNull(Task task)
        {
            if (task == null)
            {
                if (Logging.On) Logging.PrintError(Logging.Http, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpLogContentNoTaskReturnedCopytoasync, this.GetType().FullName));
                throw new InvalidOperationException(ErrorCodes.NetHttpContentNoTaskReturned);
            }
        }
        ...
        private static Exception GetStreamCopyException(Exception originalException)
        {
            ...
            {
                result = new HttpRequestException(ErrorCodes.NetHttpContentStreamCopyError, result);
            }
            return result;
        }
        ...
        private class LimitMemoryStream : MemoryStream
        {
            ...
            private void CheckSize(int countToAdd)
            {
                ...
                {
                    throw new HttpRequestException(string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpContentBuffersizeExceeded, maxSize));
                }
            }
        }
    }
