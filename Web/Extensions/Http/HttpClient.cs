...
namespace System.Net.Http
{
    public class HttpClient : IDisposable
    {
                ...
                try
                {
                    ...
                    if (response == null)
                    {
                        SetTaskFaulted(request, linkedCts, tcs,
                            new InvalidOperationException(ErrorCodes.NetHttpHandlerNoresponse));
                        return;
                    }
        ...
        private static void CheckRequestMessage(HttpRequestMessage request)
        {
            ...
            {
                throw new InvalidOperationException(ErrorCodes.NetHttpClientRequestAlreadySent);
            }
        }
        ...
        private void PrepareRequestMessage(HttpRequestMessage request)
        {
            ...
            {
                throw new InvalidOperationException(ErrorCodes.NetHttpClientInvalidRequesturi);
            }
        ...
        private static void CheckBaseAddress(Uri baseAddress, string parameterName)
        {
            ...
            {
                throw new ArgumentException(ErrorCodes.NetHttpClientAbsoluteBaseaddressRequired, parameterName);
            }

            if (!HttpUtilities.IsHttpUri(baseAddress))
            {
                throw new ArgumentException(ErrorCodes.NetHttpClientHttpBaseaddressRequired, parameterName);
            }
        }
        ...
        private void SetTaskCompleted(HttpRequestMessage request, CancellationTokenSource cancellationTokenSource,
            TaskCompletionSource<HttpResponseMessage> tcs, HttpResponseMessage response)
        {
            if (Logging.On) Logging.PrintInfo(Logging.Http, this, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpClientSendCompleted, Logging.GetObjectLogHash(request), Logging.GetObjectLogHash(response), response));
            ...
            string method, Exception e)
        {
            ...
            {
                if (Logging.On) Logging.PrintError(Logging.Http, this, method, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpClientSendCanceled, Logging.GetObjectLogHash(request)));
            }
            else
            {
                Contract.Assert(e != null);
                if (Logging.On) Logging.PrintError(Logging.Http, this, method, string.Format(System.Globalization.CultureInfo.InvariantCulture, ErrorCodes.NetHttpClientSendError, Logging.GetObjectLogHash(request), e));
            }
        }
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                tcs.TrySetException(new HttpRequestException(
                    string.Format(System.Globalization.CultureInfo.InvariantCulture,
                        ErrorCodes.NetHttpMessageNotSuccessStatuscode, (int)response.StatusCode,
